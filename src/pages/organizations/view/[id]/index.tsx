...
import { deleteOfferById } from 'apiSdk/offers';
...
function OrganizationViewPage() {
  ...
  const offerHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('offer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/offers/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.offer?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/offers/view/${record.id}`)} key={record.id}>
